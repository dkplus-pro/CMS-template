...
import { appConfig, getAppVersion, getBuildTime, TRACK_ENABLED, TRACK_SAMPLE_RATE, TRACK_ENDPOINT } from "../../config";
...
/** 内建事件类型:page_view(页面曝光)/ click(点击)/ custom(自定义) */
export type TrackEventType = "page_view" | "click" | "custom";
...
export function isSampled(rate: number | undefined, random: () => number = Math.random): boolean {
  const effective = typeof rate === "number" && Number.isFinite(rate) && rate >= 0 && rate <= 1 ? rate : 1;
