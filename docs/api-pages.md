...
| GET  | /auth/menus    | 当前用户可见菜单树(动态路由数据源) | 登录(阶段 3) |
...
| 方法   | 路径        | 说明                                   | 权限码             |
| ------ | ----------- | -------------------------------------- | ------------------ |
| GET    | /menus      | 全量菜单树(管理端)                     | system:menu:list   |
| POST   | /menus      | 新建(目录/页面),含排序、隐藏、权限绑定 | system:menu:create |
| PUT    | /menus/{id} | 编辑                                   | system:menu:update |
| DELETE | /menus/{id} | 删除(有子菜单 409,联动清权限)          | system:menu:delete |
...
- **menu 权限点**:与菜单一一对应,code 形如 `menu:system:user`,阶段 2 种子灌入、阶段 3 起由菜单管理维护;
...
| /system/menus   | 菜单管理     | 3         | 新建/编辑弹窗(目录/页面共用)             | menus 4 个                |
...
弹窗合计 9 个:用户×2、角色×2、菜单×1、字典×2、文件上传×1、修改密码×1。
...
1. 新增/变更接口:先改本清单 → 落 `openapi.yaml` → `pnpm gen:api` → 双端实现,清单与契约同一 PR;
2. 排期核对:阶段交付时按下表打勾——接口 41 个、页面 10 个路由(9 业务 + 登录)、弹窗 9 个;
