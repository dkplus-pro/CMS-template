...
6. 目录分区:`src/api`(生成物)/ `components` / `hooks` / `routes`(页面)/ `store`(全局状态)/ `utils` / `constants` / `config`;
7. 复用规则:2 个及以上页面用 → 提到 `src/components`、`src/hooks`;单页面用 → 留在页面目录内;全局共享状态 → Modern.js model(`src/store/`),不与 zustand 混用;
8. 单文件超约 300 行必须拆分,页面主入口只做数据编排。

### server(详见 [docs/server.md](docs/server.md))

9. 分层单向依赖:`handler → service → repo`;handler 薄、service 厚、repo 只管存取;
10. `main.go` 只做装配;单文件超约 400 行按资源拆分;
11. handler 实现 oapi-codegen 生成的 `ServerInterface`,一个资源一个文件;
12. server 通过自身 `package.json` 的 `dev`/`gen:api` 脚本接入 turbo,保证根命令可用。

### 通用

13. 提交信息遵循 Conventional Commits;代码风格交给仓库 prettier/eslint/gofmt 配置,不自创风格;
14. 改代码前先查 `src/components`、`src/hooks`、`internal/` 是否已有可复用实现,先复用再新建。
